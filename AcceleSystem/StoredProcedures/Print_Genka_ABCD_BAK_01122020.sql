 BEGIN TRY 
 Drop Procedure dbo.[Print_Genka_ABCD_BAK_01122020]
END try
...
-- =============================================
create PROCEDURE [dbo].[Print_Genka_ABCD_BAK_01122020]
...
BEGIN
	...
	begin
		...
		FROM (
			...
			GROUP BY YYYYMM,CostCD,CastingCD
			) T1
			PIVOT (
				SUM(Cost)
	...
	begin
		...
		FROM (
			...
			GROUP BY YYYYMM,CostCD,BrandCD,Season,Year
			) T1
			PIVOT (
				SUM(Cost) 
				FOR [YYYYMM] IN ( [01], [02], [03], [04], [05], [06], [07], [08], [09], [10], [11], [12] )
	...
	begin
		...
		FROM (
			...
			AND ( ''' + @BrandCD + ''' is null OR (BrandCD = ''' + @BrandCD + '''))
			AND ( ''' + @Season + ''' is null OR (Season IN (SUBSTRING(''' + @Season + ''',1,1),SUBSTRING(''' + @Season + ''',3,1),SUBSTRING(''' + @Season + ''',5,1))))
			GROUP BY YYYYMM,BrandCD,Season,Year,ProjectCD,HinbanCD,CostCD
			) T1
			PIVOT (
				SUM(Cost)
				FOR [YYYYMM] IN ( [01], [02], [03], [04], [05], [06], [07], [08], [09], [10], [11], [12] )
	...
	begin
		...
		FROM (
			...
			' GROUP BY YYYYMM, CostCD, ProjectCD, HinbanCD, Cost
			) T1
			PIVOT (
				SUM(Cost)
