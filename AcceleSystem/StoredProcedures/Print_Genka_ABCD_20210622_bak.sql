 BEGIN TRY 
 Drop Procedure dbo.[Print_Genka_ABCD_20210622_bak]
END try
...
-- =============================================
CREATE PROCEDURE [dbo].[Print_Genka_ABCD_20210622_bak]
...
BEGIN
	...
	begin				
		...
		SELECT ''金型費'', t1.CastingCD, ISNULL(mc.CastingName,''''), t1.CostCD, mk.CostName, isnull(t2.Cost, ''''), isnull(t1.[01], ''''), isnull(t1.[02], ''''), isnull(t1.[03], ''''), isnull(t1.[04], ''''),				
			isnull(t1.[05], ''''), isnull(t1.[06], ''''), isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 0, 0 + ISNULL(t2.Cost, 0), 0, 10
		FROM (
			SELECT
	...
	begin
		SET @SQL = 'INSERT INTO WK_原価集計表_' + @LoginID + ' ( CostName, CostPre, Cost1,Cost2, Cost3, Cost4, Cost5, Cost6, Cost7, Cost8, Cost9, Cost10, Cost11, Cost12, CostHaihu, CostHimoku, CostShikakari, SortKey)
					...
					sum(CostPre) + isnull(Cost1, 0) + isnull(Cost2, 0) + isnull(Cost3, 0) + isnull(Cost4, 0) +	isnull(Cost5, 0) + isnull(Cost6, 0) + isnull(Cost7, 0) + isnull(Cost8, 0) + isnull(Cost9, 0) + 
					isnull(Cost10, 0) + isnull(Cost11, 0) + isnull(Cost12, 0), 0, 11
	...
	begin
		SET @SQL = 'INSERT INTO WK_原価集計表_' + @LoginID + ' ( CostName, CostPre, Cost1, Cost2, Cost3, Cost4, Cost5, Cost6, Cost7, Cost8, Cost9, Cost10, Cost11, Cost12, CostHaihu,CostHimoku, CostShikakari, SortKey)		
					SELECT ''B/S合計(直接) '', sum(CostPre), sum(Cost1), sum(Cost2), sum(Cost3), sum(Cost4), sum(Cost5), sum(Cost6), sum(Cost7), sum(Cost8), sum(Cost9), sum(Cost10), sum(Cost11), sum(Cost12), 0, sum(CostPre) + 0, 0, 21
					FROM  WK_原価集計表_' + @LoginID +
	...
	begin
		...
		--EXEC (@SQL);
		EXECUTE sp_executesql @SQL, N'@BrandCD varchar(10), @BrandName varchar(40), @ProjectCD varchar(10), @ProjectName varchar(60)', 
	...
	begin
		...
		) t2 ON LEFT(t1.YYYYMM, 4) = t2.YYYY AND t1.CostCD = t2.CostCD 
		LEFT OUTER JOIN
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, left(mp.PeriodStart,4) +''/''+ right(mp.PeriodStart,2) AS PeriodStart, left(mp.PeriodEnd,4) +''/''+ right(mp.PeriodEnd,2) AS PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, h.HinbanName, h.Color, 
			...
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 
			isnull(t2.Cost, 0) + isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as,
			0 as CostShikakari, 
			4106 as SortKey
		FROM (
			SELECT
