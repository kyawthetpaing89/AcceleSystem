 BEGIN TRY 
 Drop Procedure dbo.[Print_Genka_ProjectData_20210622_bak]
END try
...
-- =============================================
CREATE PROCEDURE [dbo].[Print_Genka_ProjectData_20210622_bak]
...
BEGIN
	...
	BEGIN
		...
		SET @SQL = 'SELECT @cnt=CASE WHEN COUNT(*) > 0 THEN COUNT(*) ELSE 0 END FROM (SELECT DISTINCT ProjectCD FROM WK_原価集計表_' + @LoginID + ' WHERE SortKey = 30 AND BrandCD = @BrandCD AND Season = @Season GROUP BY ProjectCD) C'
		...
		SUM(Cost6) AS Cost6, SUM(Cost7) AS Cost7, SUM(Cost8) AS Cost8, SUM(Cost9) AS Cost9, SUM(Cost10) AS Cost10, SUM(Cost11) AS Cost11, SUM(Cost12) AS Cost12, SUM(CostHaihu) AS CostHaihu, SUM(CostHimoku) AS CostHimoku, SUM(CostShikakari) AS CostShikakari, SortKey, RIGHT(mc.FiscalYYYYMM, 2) as FiscalMM,' + cast((@b_count + @c_count + @d_count) as varchar) + ' as Row_Count
		...
		SUM(Cost3), SUM(Cost4), SUM(Cost5), SUM(Cost6), SUM(Cost7), SUM(Cost8), SUM(Cost9), SUM(Cost10), SUM(Cost11), SUM(Cost12), SUM(CostHaihu),SUM(CostHimoku), SUM(CostShikakari), ''21'' AS SortKey, NULL,' + cast((@b_count + @c_count + @d_count) as varchar) + ' as Row_Count
		...
		null, null, null,  null, null, null,  null, null, null,  null, 0 as CostHimoku,null, SortKey, RIGHT(mc.FiscalYYYYMM, 2) as FiscalMM, ' + cast((@b_count + @c_count + @d_count) as varchar) + ' as Row_Count
		FROM WK_原価集計表_' + @LoginID + ' wk, M_Control mc 
