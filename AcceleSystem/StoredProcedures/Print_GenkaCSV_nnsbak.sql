 BEGIN TRY 
 Drop Procedure dbo.[Print_GenkaCSV_nnsbak]
END try
...
-- =============================================
CREATE PROCEDURE [dbo].[Print_GenkaCSV_nnsbak]
...
BEGIN
	...
	DECLARE @loopcount INT, @count as int, @rowcount INT, @a_count INT, @o_count INT, @b_count INT, @t_count INT, @minid INT, @maxid INT, @prjCD VARCHAR(10), @hinbanCD VARCHAR(10);
	...
	BEGIN		
		...
		EXECUTE sp_executesql @SQL, N'@cnt int OUTPUT, @prjCD varchar(10) OUTPUT, @hinbanCD varchar(10) OUTPUT, @minid int', @cnt=@loopcount OUTPUT, @prjCD = @prjCD OUTPUT, @hinbanCD = @hinbanCD OUTPUT, @minid = @minid
		WHILE (@count <= @loopcount) 
		BEGIN
	...
	--SET @SQL = 'SELECT @cnt=COUNT(*) FROM WK_原価集計表_' + @LoginID + ' WHERE SortKey <> 10 AND SortKey <> 11 AND ((SortKey = 20 OR SortKey = 21) OR ((SortKey = 30 OR SortKey = 31 OR SortKey = 32) OR 
	...
	SET @SQL = 'SELECT @cnt=COUNT(*) FROM WK_原価集計表_' + @LoginID + ' WHERE SortKey <> 10 AND SortKey <> 11'
	...
	SET @SQL = 'SELECT @cnt=COUNT(*) + 1 FROM (SELECT DISTINCT ProjectCD FROM WK_原価集計表_' + @LoginID + ' WHERE SortKey = 30) T'
