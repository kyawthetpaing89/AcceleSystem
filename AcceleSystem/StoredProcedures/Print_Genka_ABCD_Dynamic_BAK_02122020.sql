 BEGIN TRY 
 Drop Procedure dbo.[Print_Genka_ABCD_Dynamic_BAK_02122020]
END try
...
-- =============================================
create PROCEDURE [dbo].[Print_Genka_ABCD_Dynamic_BAK_02122020]
...
BEGIN
	...
	begin
		...
		SELECT ''金型費'', t1.CastingCD, mc.CastingName, t1.CostCD, mk.CostName, isnull(t2.Cost, ''''), isnull(t1.[01], ''''), isnull(t1.[02], ''''), isnull(t1.[03], ''''), isnull(t1.[04], ''''),
			isnull(t1.[05], ''''), isnull(t1.[06], ''''), isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 0, 0, 10
		FROM (
			SELECT
	...
	begin
		...
		SELECT t1.BrandCD, mb.BrandName, t1.Season, t1.Year, t1.CostCD, mk.CostName, isnull(t2.Cost, ''''), isnull(t1.[01], ''''), isnull(t1.[02], ''''), isnull(t1.[03], ''''), isnull(t1.[04], ''''),
			isnull(t1.[05], ''''), isnull(t1.[06], ''''), isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 0, 0, 20
		FROM (
			SELECT
				right(YYYYMM,2) as YYYYMM,
				BrandCD,
	...
	begin
		...
		SELECT t1.BrandCD, mb.BrandName, t1.Season, t1.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum,
		...
		isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 30 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as YYYYMM,
				BrandCD,
		...
		LEFT OUTER JOIN
			(SELECT HinbanCD, ProjectCD, Production, sum(isnull(Production, 0)) as TotalValueSum, sum(isnull(SalesPrice, 0) * isnull(Production, 0)) as ProductionSum 
			 FROM M_Hinban
			 GROUP BY HinbanCD, ProjectCD, Production) mh ON t1.HinbanCD = mh.HinbanCD AND mp.ProjectCD = mh.ProjectCD
		LEFT OUTER JOIN
		...
		AND (( ''' + @DeliveryStatus + ''' = ''1'' and dd.DeliveryAmount < mh.Production) OR ( ''' + @DeliveryStatus + ''' = ''2'' and dd.DeliveryAmount = mh.Production))
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			mh.Production, mh.Complete, mh.SalesPrice, mh.TotalValue, mh.Casting, mc.CastingName, t1.CostCD, mk.CostName, isnull(t2.Cost, ''''), isnull(t1.[01], ''''), isnull(t1.[02], ''''), isnull(t1.[03], ''''), 
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 40 as SortKey
		FROM (
			SELECT
		...
		LEFT OUTER JOIN	
		(
			SELECT d1.YYYY,d1.CostCD,d1.ProjectCD, d1.HinbanCD,sum(d1.Cost) Cost from D_TotalDirectCost d1
			INNER JOIN (
						SELECT max(YYYY) as YYYY, CostCD, ProjectCD, HinbanCD, Cost
						FROM D_TotalDirectCost
						WHERE YYYY < ' + @targetyear + ' GROUP BY CostCD, ProjectCD, HinbanCD, Cost
					) d2 ON d1.YYYY = d2.YYYY AND d1.CostCD = d2.CostCD AND d1.ProjectCD = d2.ProjectCD AND d1.HinbanCD = d2.HinbanCD
			WHERE d1.YYYY < ' + @targetyear + ' GROUP BY d1.YYYY, d1.CostCD, d1.ProjectCD, d1.HinbanCD
		) t2 ON LEFT(t1.YYYYMM, 4) = t2.YYYY AND t1.ProjectCD = t2.ProjectCD 
		LEFT OUTER JOIN M_Keihi mk on t1.CostCD = mk.CostCD
		LEFT OUTER JOIN M_Project mp on t1.ProjectCD = mp.ProjectCD
		LEFT OUTER JOIN
		...
		LEFT OUTER JOIN
			(SELECT ProjectCD, HinbanCD, SUM(isnull(DeliveryAmount, 0)) as DeliveryAmount
			 FROM D_Delivery
			 GROUP BY ProjectCD, HinbanCD) dd ON mh.HinbanCD = dd.HinbanCD AND mh.ProjectCD = dd.ProjectCD
		LEFT OUTER JOIN M_Casting mc ON mh.Casting = mc.CastingCD
	...
	begin
		SET @SQL = 'INSERT INTO WK_原価集計表_' + @LoginID + ' ( BrandCD, BrandName, Season, Year, ProjectCD, ProjectName, PeriodStart, PeriodEnd, TotalValueSum, ProductionSum, HinbanCD, HinbanName,
			Color, Production, Complete, SalesPrice, TotalValue, Casting, CastingName, CostCD, CostName, CostPre, Cost1, Cost2, Cost3, Cost4, Cost5, Cost6, Cost7, Cost8, Cost9, Cost10, Cost11, Cost12, CostHaihu, CostShikakari, SortKey )
		SELECT t1.BrandCD, mb.BrandName, t1.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			mh.Production, mh.Complete, mh.SalesPrice, mh.TotalValue, mh.Casting, mc.CastingName, t1.CostCD, mk.CostName, isnull(t2.Cost, ''''), isnull(t1.[01], ''''), isnull(t1.[02], ''''), isnull(t1.[03], ''''), 
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4100 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
		...
		) t2 ON t1.CostCD = t2.CostCD AND t1.BrandCD = t2.BrandCD AND mh.Casting = t2.CastingCD
		WHERE ( ''' + @BrandCD + ''' is null OR (mp.BrandCD = ''' + @BrandCD + ''' ))
		AND (''' + @BrandName + ''' is null OR (mb.BrandName like ''%'' + ''' + @BrandName + ''' +''%''))
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4101 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
		...
		(
			...
			WHERE d1.Accounting = 3 AND left(d1.YYYYMM,4) < ' + @targetyear + '
		) t2 ON t1.CostCD = t2.CostCD AND mp.BrandCD = t2.BrandCD AND mh.Casting = t2.CastingCD
		WHERE ( ''' + @BrandCD + ''' is null OR (mp.BrandCD = ''' + @BrandCD + ''' ))
		AND (''' + @BrandName + ''' is null OR (mb.BrandName like ''%'' + ''' + @BrandName + ''' +''%''))
		AND ( ''' + @Season + ''' is null OR (mp.Season IN (SUBSTRING( ''' + @Season + ''',1,1),SUBSTRING( ''' + @Season + ''',3,1),SUBSTRING( ''' + @Season + ''',5,1))))
		AND (''' + @ProjectCD + ''' is null or (mp.ProjectCD = ''' + @ProjectCD + '''))
	...
	begin
		SET @SQL = 'INSERT INTO WK_原価集計表_' + @LoginID + ' ( BrandCD, BrandName, Season, Year, ProjectCD, ProjectName, PeriodStart, PeriodEnd, TotalValueSum, ProductionSum, HinbanCD, HinbanName,
			Color, Production, Complete, SalesPrice, TotalValue, Casting, CastingName, CostCD, CostName, CostPre, Cost1, Cost2, Cost3, Cost4, Cost5, Cost6, Cost7, Cost8, Cost9, Cost10, Cost11, Cost12, CostHaihu, CostShikakari, SortKey )
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			mh.Production, mh.Complete, mh.SalesPrice, mh.TotalValue, t1.CastingCD, mc.CastingName, t1.CostCD, mk.CostName, isnull(t2.Cost, ''''), isnull(t1.[01], ''''), isnull(t1.[02], ''''), isnull(t1.[03], ''''), 
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4102 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4103 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT t1.BrandCD, mb.BrandName, t1.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4104 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4105 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4106 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4107 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT t1.BrandCD, mb.BrandName, t1.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4108 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4109 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4110 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
	...
	begin
		...
		SELECT mp.BrandCD, mb.BrandName, mp.Season, mp.Year, t1.ProjectCD, mp.ProjectName, mp.PeriodStart, mp.PeriodEnd, mh.TotalValueSum, mh.ProductionSum, t1.HinbanCD, mh.HinbanName, mh.Color, 
			...
			isnull(t1.[04], ''''), isnull(t1.[05], ''''), isnull(t1.[06], ''''),  isnull(t1.[07], ''''), isnull(t1.[08], ''''), isnull(t1.[09], ''''), isnull(t1.[10], ''''), isnull(t1.[11], ''''), isnull(t1.[12], ''''), 
			isnull(t1.[01], 0) + isnull(t1.[02], 0) + isnull(t1.[03], 0) + isnull(t1.[04], 0) +	isnull(t1.[05], 0) + isnull(t1.[06], 0) + isnull(t1.[07], 0) + isnull(t1.[08], 0) + isnull(t1.[09], 0) + 
			isnull(t1.[10], 0) + isnull(t1.[11], 0) + isnull(t1.[12], 0) as CostHaihu, 0 as CostShikakari, 4111 as SortKey
		FROM (
			SELECT
				right(YYYYMM,2) as MM,
				CostCD,
