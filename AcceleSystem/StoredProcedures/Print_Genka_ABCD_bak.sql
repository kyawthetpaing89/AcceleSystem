 BEGIN TRY 
 Drop Procedure dbo.[Print_Genka_ABCD_bak]
END try
...
BEGIN
	...
	begin
		SET @SQL = 'INSERT INTO WK_原価集計表_' + @LoginID + ' ( CostName, CostPre, Cost1,Cost2, Cost3, Cost4, Cost5, Cost6, Cost7, Cost8, Cost9, Cost10, Cost11, Cost12, CostHaihu, CostHimoku, CostShikakari, SortKey)
					SELECT ''金型合計'', sum(CostPre), sum(Cost1), sum(Cost2), sum(Cost3), sum(Cost4), sum(Cost5), sum(Cost6), sum(Cost7), sum(Cost8), sum(Cost9), sum(Cost10), sum(Cost11), sum(Cost12), 0, sum(CostPre) + 0, 0, 11
